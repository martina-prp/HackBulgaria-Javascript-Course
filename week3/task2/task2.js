...
$(document).ready(function() {
  ...
  var groupGroceries = groupBy(function(grocery) {
    return grocery.type;
  }, groceries);

  var template = $("#single-table-row").html();
  var parsedFruits = groupGroceries.fruit.map(function(x) {
    return _.template(template, x);
  });
  var parsedVegetables = groupGroceries.vegetable.map(function(x) {
    return _.template(template, x);
  });
  ...
  $(".vegetable-table").append([
    '<table>',
    parsedVegetables.join(""),
    '</table>'
  ].join(""));
